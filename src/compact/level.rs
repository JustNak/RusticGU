//! User-facing compact strength for the library dialog.
//!
//! High = LZX is a **per-action** override. The live Settings picker stays
...
/// Three-choice compact strength shown in the library dialog.
...
impl CompactLevel {
    pub const ALL: [CompactLevel; 3] =
        [CompactLevel::Low, CompactLevel::Medium, CompactLevel::High];
...
mod tests {
    ...
    #[test]
    fn levels_map_to_xpress4k_xpress8k_lzx() {
        ...
        assert_eq!(CompactLevel::High.algorithm(), CompactAlgorithm::Lzx);
        assert_eq!(CompactLevel::High.algorithm().exe_flag(), "LZX");
    ...
    fn high_lzx_apply_uses_include_set_not_root_s() {
        ...
        let algo = CompactLevel::High.algorithm();
...
"#,
    ...
    fn live_picker_still_excludes_lzx() {
        ...
        assert_eq!(CompactLevel::High.algorithm().label(), "LZX");
        let _ = PathBuf::new();
    }
}
